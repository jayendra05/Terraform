...
# Define a public IP address for the first VM (Server01)
...
# Define a public IP address for the second VM (demo-server01)
resource "azurerm_public_ip" "my_demo_vm_public_ip" {
  name                = "myPublicIP-DemoVM"
  location            = azurerm_resource_group.my_resource_group.location
  resource_group_name = azurerm_resource_group.my_resource_group.name
  allocation_method   = "Dynamic"
}

# Network interface for the first VM (Server01)
...
# Network interface for the second VM (demo-server01)
resource "azurerm_network_interface" "my_demo_vm_nic" {
  name                = "myDemoVMNIC"
  location            = azurerm_resource_group.my_resource_group.location
  resource_group_name = azurerm_resource_group.my_resource_group.name

  ip_configuration {
    name                          = "myDemoVMNICConfig"
    subnet_id                     = azurerm_subnet.my_subnet.id
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id           = azurerm_public_ip.my_demo_vm_public_ip.id
  }
}
...
resource "azurerm_virtual_machine" "my_demo_vm" {
  ...
  resource_group_name   = azurerm_resource_group.my_resource_group.name
  availability_set_id   = azurerm_availability_set.my_availability_set.id
  network_interface_ids = [azurerm_network_interface.my_demo_vm_nic.id]
  vm_size               = "Standard_B2s"
